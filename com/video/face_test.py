...
def handwritingClassTest():
    hwLabels=[]
    trainingFileList=os.listdir(r"F:\train_pic")
    m=len(trainingFileList)
    trainingMat=np.zeros((m,200*200*3))
    for i in range(m):
        fileNameStr=trainingFileList[i]
        classNumber=int(fileNameStr.split('_')[0])
        hwLabels.append(classNumber)
        trainingMat[i,:]=img2vector2(r'F:\train_pic\%s'%(fileNameStr))
    ...
    testFileList=os.listdir(r'F:\test_pic')
    ...
    for i in range(mTest):
        ...
        classNumber=int(fileNameStr.split('_')[0])
        vectorUnderTest=img2vector2(r'F:\test_pic\%s'%(fileNameStr))
